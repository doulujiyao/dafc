...
from fsgan.data.landmark_transforms import LandmarksTransform,Pyramids
...
def read_bboxes(bboxes_file,st,end):
    if bboxes_file is None:
        return None
    if end==-1:
        return np.load(bboxes_file)[st:]    
    if bboxes_file.lower().endswith('.npy'):
        return np.load(bboxes_file)[st:end]
...
class FaceLandmarksDataset(ImageListDataset):
    ...
    """
    def __init__(self, st,end,root, img_list, landmarks_list=None, bboxes_list=None,
                 transform=None, target_transform=None, loader=default_loader):
        super(FaceLandmarksDataset, self).__init__(st,end,root, img_list, transform, target_transform, loader)
        landmarks_list_path = '/home/dafc/HRNet-Facial-Landmark-Detection/output_final/WFLW/face_alignment_wflw_hrnet_w18/predictions2.npy' if os.path.exists(landmarks_list) else os.path.join(root, landmarks_list)
        ...
        self.landmarks = read_landmarks(landmarks_list_path,st,end)
        self.bboxes = read_bboxes(bboxes_list_path,st,end)
...
class FacePairRefSegLandmarksDataset(FaceLandmarksDataset):
    ...
    Args:
     ...
     Attributes:
        ...
        bboxes (numpy.ndarray): Array of bounding boxes per image.
    """
    def __init__(self, st,end,root, img_list, landmarks_list=None, bboxes_list=None, pair_transform=None,
                 transform1=None, transform2=None, target_transform=None, loader=default_loader, same_prob=3):
        super(FacePairRefSegLandmarksDataset, self).__init__(st,end,root,img_list, landmarks_list, bboxes_list,
        ...
        paths, labels ,path_seg= zip(*self.imgs)
        self.label_ranges = [len(self.imgs)] * (len(self.class_to_idx) + 1)
        for i, img in enumerate(self.imgs): #这是按照一定的顺序的
    ...
    def read_data(self, index):
        path, label, path_seg= self.samples[index]
        img = self.loader(path)
        landmarks = self.landmarks[index] if self.landmarks is not None else None
        
        #img_seg=self.loader(path_seg)
        ...
        preds=landmarks
        #print(preds.shape)
        img_seg=np.zeros([720,1280,3],dtype=np.uint8)
        
        
        
        #points=np.array([[150,50],[140,140],[200,170],[250,250]],np.int32)  #多边形的顶点坐标
        
        #points=np.array([preds[0,76,:].numpy(),preds[0,77,:].numpy(),preds[0,78,:].numpy(),preds[0,79,:].numpy(),preds[0,80,:].numpy(),preds[0,81,:].numpy(),preds[0,82,:].numpy(),preds[0,83,:].numpy(),preds[0,84,:].numpy(),preds[0,85,:].numpy(),preds[0,86,:].numpy(),preds[0,87,:].numpy()],np.int32)
        points=np.array([preds[76:88,:]],np.int32)
        #print(points)
        #points = preds[0,76:88,:].numpy()
        points=points.reshape(-1,1,2)
        cv2.fillPoly(img_seg,[points],(255,255,255))  #画任意多边形
        points=np.array([preds[60:68,:]],np.int32)
        #print(points)
        #points = preds[0,76:88,:].numpy()
        points=points.reshape(-1,1,2)
        cv2.fillPoly(img_seg,[points],(255,255,255))  #画任意多边形
        points=np.array([preds[68:76,:]],np.int32)
        #print(points)
        #points = preds[0,76:88,:].numpy()
        points=points.reshape(-1,1,2)
        cv2.fillPoly(img_seg,[points],(255,255,255))  #画任意多边形

        points=np.array([preds[51,:],preds[55,:],preds[56,:],preds[57,:],preds[58,:],preds[59,:]],np.int32)
        #print(points)
        #points = preds[0,76:88,:].numpy()
        points=points.reshape(-1,1,2)
        cv2.fillPoly(img_seg,[points],(255,255,255))  #画任意多边形
        img_seg = Image.fromarray(cv2.cvtColor(img_seg,cv2.COLOR_BGR2RGB))  
        ...
        return img, landmarks, img_seg, label
    ...
    def __getitem__(self, index):
        ...
        for ref_i in range(ref_num):
            indexref.append(self.select_second_index(index))
        img1, landmarks1, img_seg1, label1 = self.read_data(index)
        img2, landmarks2, img_seg2, label2 = self.read_data(index2)
        imgref=[]
        landmarksref=[]
        img_segref=[]
        labelref=[]
        for ref_i in range(ref_num):
            imgtmp,landmarkstmp,img_segtmp,labeltmp=self.read_data(indexref[ref_i])
            imgtmp=np.array(imgtmp)
            imgtmp_e=np.where(img_segtmp,imgtmp,img_segtmp)
            imgtmp=Image.fromarray(imgtmp_e)

            imgref.append(imgtmp)
            landmarksref.append(landmarkstmp)

        img_seg1=np.array(img_seg1)
        img_seg2=np.array(img_seg2)
        img1=np.array(img1)
        
        img_se=np.where(img_seg1<10,img1,0)
        img_e=np.where(img_seg1,img1,img_seg1)
        img_se=Image.fromarray(img_se)
        img_e=Image.fromarray(img_e)
        img1=Image.fromarray(img1)
        imgref.append(img_se)
        imgref.append(img_e)
        #landmarksref.append(landmarks1)
        #landmarksref.append(landmarks1)

        img2=np.array(img2)
        img_se=np.where(img_seg2<10,img2,0)
        img_e=np.where(img_seg2,img2,img_seg2)
        img_se=Image.fromarray(img_se)
        img_e=Image.fromarray(img_e)
        img2=Image.fromarray(img2)
        imgref.append(img_se)
        imgref.append(img_e)
        #landmarksref.append(landmarks2)
        #landmarksref.append(landmarks2)
        img_seg1=Image.fromarray(img_seg1)
        img_seg2=Image.fromarray(img_seg2)


            
        if self.pair_transform is not None:
            img1, landmarks1, img_seg1, img2, landmarks2, img_seg2,imgref,landmarksref = self.pair_transform(
                img1, landmarks1, img_seg1, img2, landmarks2, img_seg2,imgref,landmarksref)
        if self.transform1 is not None:
            img1, landmarks1, img_seg1,imgref,landmarksref = self.transform1(img1, landmarks1, img_seg1,imgref,landmarksref)
        if self.transform2 is not None:
            img2, landmarks2, img_seg2 = self.transform2(img2, landmarks2, img_seg2)
        if self.target_transform is not None:
            label1, label2 = self.target_transform(label1), self.target_transform(label2)

        return img1, landmarks1, img_seg1, label1, img2, landmarks2, img_seg2, label2,imgref,landmarksref



class FacePairSegLandmarksDataset(FaceLandmarksDataset):
    """A face list dataset for loading face pairs with landmarks and bounding boxes.
    ...
    def __init__(self,st,end, root, img_list, landmarks_list=None, bboxes_list=None, pair_transform=None,
                 transform1=None, transform2=None, target_transform=None, loader=default_loader, same_prob=3):
        super(FacePairSegLandmarksDataset, self).__init__(st,end,root,img_list, landmarks_list, bboxes_list,
                                                       None, target_transform, loader)
        self.pair_transform = pair_transform
        self.transform1 = transform1
        self.transform2 = transform2
    ...
    def __getitem__(self, index):
        ...
        img2, landmarks2, img_seg2, label2 = self.read_data(index2)
        if self.pair_transform is not None:
            img1, landmarks1, img_seg1, img2, landmarks2, img_seg2 = self.pair_transform(
                img1, landmarks1, img_seg1, img2, landmarks2, img_seg2)
        if self.transform1 is not None:
            img1, landmarks1, img_seg1 = self.transform1(img1, landmarks1, img_seg1)
        if self.transform2 is not None:
            img2, landmarks2, img_seg2 = self.transform2(img2, landmarks2, img_seg2)
        if self.target_transform is not None:
            label1, label2 = self.target_transform(label1), self.target_transform(label2)

        return img1, landmarks1, img_seg1, label1, img2, landmarks2, img_seg2, label2
