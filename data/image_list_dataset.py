...
def make_dataset(root, img_rel_paths, class_to_idx):
    ...
    for img_rel_path in img_rel_paths:
        ...
        images.append((img_rel_path, class_to_idx[target],img_rel_path.replace('Video','Video_landmark_seg')))

    return images


def calc_weights_for_balanced_classes(samples, num_classes):
    paths, labels,path_seg = zip(*samples)
...
class ImageListDataset(DatasetFolder):
    ...
    """
    def __init__(self, st,end,root, img_list, transform=None, target_transform=None, loader=default_loader):
        ...
        if end==-1:
            img_rel_paths = sorted(img_rel_paths)[st:]
        else:
            img_rel_paths = sorted(img_rel_paths)[st:end]   # Make sure the paths are in the right order
